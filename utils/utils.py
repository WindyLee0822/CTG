...
def reduce_mean(value, mask, axis=None):
    if axis is None:
        return (torch.sum(value * mask)+1e-8)/ (torch.sum(mask)+1e-8)
    return (reduce_sum(value, mask, axis)+1e-8) / (torch.sum(mask, axis)+1e-8)
...
def distinctness(generations):
    unigrams, bigrams, trigrams, forgrams = set(), set(), set(), set()
    total_words = 0
    for gen in generations:
        ...
        for i in range(len(o) - 3):
            forgrams.add(o[i] + '_' + o[i + 1] + '_' + o[i + 2] + '_' + o[i + 3])

    return len(unigrams) / total_words, len(bigrams) / total_words, len(trigrams) / total_words, len(forgrams) / total_words
